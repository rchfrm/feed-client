...
const GetStartedConnectFacebook = () => {
  ...
  const [selectedProfile, setSelectedProfile] = React.useState<Nullable<string>>(null)
  ...
  useAsyncEffect(async (isMounted) => {
    ...
    const processedArtists = artistHelpers.processArtists(artistsFiltered)
    ...
    if (processedArtists.length === 1 && isPlatformRedirect && ! artistId) {
      ...
      setSelectedProfile(processedArtists[0].name)
  ...
  if (isConnecting && artistAccounts.length > 0) {
    return <ConnectProfilesIsConnecting profileName={selectedProfile} className="my-6 sm:my-0" />
  }
